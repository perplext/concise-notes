...
import { useTranscribeKeyboardShortcuts, getModifierKeyLabel } from '../hooks/useKeyboardShortcuts';
...
function TranscribePage({ config }: TranscribePageProps) {
  const getProgressMessage = (progress: number): string => {
    if (progress < 10) return 'Preparing transcription...';
    if (progress < 20) return 'Loading model...';
    if (progress < 30) return 'Processing audio file...';
    if (progress < 40) return 'Converting audio format...';
    if (progress < 50) return 'Reading audio data...';
    if (progress < 60) return 'Preparing for transcription...';
    if (progress < 80) return 'Transcribing with Whisper...';
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (fileSizeGB > maxSizeGB) {
        alert(`File too large: ${fileSizeGB.toFixed(2)} GB exceeds maximum of ${maxSizeGB} GB.\n\nFor best results, consider splitting very long videos into parts.`);
      ...
      if (fileSizeGB > 5) {
        const proceed = confirm(`Large file detected: ${fileSizeGB.toFixed(2)} GB\n\nTranscription of files this large may take considerable time (potentially hours).\n\nDo you want to continue?`);
        if (!proceed) {
          event.target.value = ''; // Clear the input
          return;
        }
      ...
      setSelectedFilePath(null); // Clear Electron file path
      setSelectedFileSize(null); // Clear custom file size
    }
  };
  ...
  const handleElectronFileSelect = async () => {
    ...
    if (filePath) {
      ...
      // Get actual file info from Electron
      const fileInfo = await electronService.getFileInfo(filePath);
      
      const fileName = filePath.split('/').pop() || filePath.split('\\').pop() || 'Selected file';
      
      if (fileInfo && fileInfo.size) {
        console.log('File info:', {
          path: filePath,
          sizeBytes: fileInfo.size,
          sizeMB: (fileInfo.size / (1000 * 1000)).toFixed(2) + ' MB (decimal)',
          sizeMiB: (fileInfo.size / (1024 * 1024)).toFixed(2) + ' MiB (binary)',
          sizeGB: (fileInfo.size / (1024 * 1024 * 1024)).toFixed(2) + ' GB'
        });
        setSelectedFileSize(fileInfo.size);
        ...
        // Fallback to estimate if we can't get actual size
        const ext = fileName.split('.').pop()?.toLowerCase();
        let estimatedSize = 10 * 1024 * 1024; // Default 10MB
        
        if (ext === 'mp4' || ext === 'mov' || ext === 'avi') {
          estimatedSize = 100 * 1024 * 1024; // 100MB for video
        } else if (ext === 'mp3' || ext === 'wav' || ext === 'm4a') {
          estimatedSize = 20 * 1024 * 1024; // 20MB for audio
        }
        
        setSelectedFileSize(estimatedSize);
      }
  ...
  const downloadTranscript = (includeSummary: boolean = false) => {
    if (!processedResult) return;
    
    let content = activeTab === 'formatted' && processedResult.formatted_text 
      ? processedResult.formatted_text
      : activeTab === 'summary' && processedResult.summary
      ? processedResult.summary.content
      : processedResult.original_text;
    
    // Add summary if requested and available
    if (includeSummary && processedResult.summary) {
      content += '\n\n\n===== SUMMARY =====\n\n';
      content += processedResult.summary.content;
      
      if (processedResult.summary.key_points && processedResult.summary.key_points.length > 0) {
        content += '\n\nKey Points:\n';
        processedResult.summary.key_points.forEach(point => {
          content += `• ${point}\n`;
        });
      }
      
      if (processedResult.summary.action_items && processedResult.summary.action_items.length > 0) {
        content += '\n\nAction Items:\n';
        processedResult.summary.action_items.forEach(item => {
          content += `☐ ${item}\n`;
        });
      }
    }
  ...
  const copyToClipboard = async (includeSummary: boolean = false) => {
    if (!processedResult) return;
  ...
  const handleTranscribe = async () => {
    ...
    // Check if we're in Electron and use native transcription
    if (electronService.isElectron && selectedFilePath) {
      try {
        // Use the already selected file path
        const filePath = selectedFilePath;
        
        // Set up progress listener
        const unsubscribe = electronService.onProgress((progress) => {
          // Update progress based on the stage and progress value
        ...
        const result = await electronService.transcribeFile(filePath, {
        ...
        if (result && result.success) {
          // Pass the entire result or just the text
          await completeTranscription(result.text || result.segments || result);
        } else {
          console.error('Transcription failed:', result?.error);
          setIsTranscribing(false);
        }
      } catch (error) {
        console.error('Electron transcription error:', error);
        setIsTranscribing(false);
      }
    } else {
      // Fallback to simulated transcription for browser mode
  ...
  const completeTranscription = async (actualTranscript?: string | any) => {
    setIsTranscribing(false);
    
    // Extract text from the transcript result
    let rawTranscript: string;
    
    if (actualTranscript) {
      // Handle both string and object with segments
      if (typeof actualTranscript === 'string') {
        rawTranscript = actualTranscript;
      } else if (Array.isArray(actualTranscript)) {
        // If it's an array of segments, join their text
        rawTranscript = actualTranscript.map((segment: any) => 
          typeof segment === 'string' ? segment : segment.text || ''
        ).join(' ');
      } else if (actualTranscript.text) {
        // If it's an object with a text property
        rawTranscript = actualTranscript.text;
      } else if (actualTranscript.segments) {
        // If it has segments property
        rawTranscript = actualTranscript.segments.map((s: any) => s.text || '').join(' ');
      } else {
        // Fallback to string conversion
        rawTranscript = String(actualTranscript);
      }
    } else {
      // Demo text for browser mode
      rawTranscript = `This is a sample transcription result from the audio file. It contains the spoken content but might need formatting and could benefit from summarization. The transcript includes various topics and key points that could be extracted for better understanding. This demonstrates how the post-processing pipeline can improve readability and extract valuable insights from the raw transcription output.
...
The content covers multiple important topics including technology implementation challenges and solutions. There are several action items mentioned throughout the discussion that need to be followed up on. The speakers also shared valuable insights about best practices and lessons learned from their experience.`;
  ...
  useTranscribeKeyboardShortcuts(
    () => downloadTranscript(false), // Save/download handler
    handleTranscribe, // Start transcription handler
    isTranscribing // Is currently transcribing
  );
                  ...
                  <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          {(() => {
                            const size = selectedFileSize || selectedFile.size || 0;
                            const sizeGB = size / (1024 * 1024 * 1024);
                            const sizeMB = size / (1024 * 1024);
                            
                            if (sizeGB >= 1) {
                              return `${sizeGB.toFixed(2)} GB${sizeGB > 5 ? ' ⚠️ Large file' : ''}`;
                            } else {
            ...
            <div className="card">
              <div className="card-body">
                ...
                <button
                  ...
                  title={`Start transcription (${getModifierKeyLabel()}+Enter)`}
                >
                  {isTranscribing ? (
                    <>
                ...
                <div className="card-body">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      <p>• Audio extraction: {progress >= 40 ? 'Complete' : progress >= 30 ? 'In progress...' : 'Pending'}</p>
                      <p>• Model loading: {progress >= 20 ? 'Complete' : progress >= 10 ? 'In progress...' : 'Pending'}</p>
                      <p>• Transcription: {progress >= 95 ? 'Complete' : progress >= 60 ? 'In progress...' : 'Pending'}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {processedResult ? (
                  <div className="space-y-4">
                    ...
                    <div className="min-h-[200px] max-h-[400px] overflow-y-auto">
                      {activeTab === 'original' && (
                        <div className="whitespace-pre-line text-sm text-gray-700 dark:text-gray-300 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                          {processedResult.original_text}
                        </div>
                      )}
                      
                      {activeTab === 'formatted' && processedResult.formatted_text && (
                        <div className="whitespace-pre-line text-sm text-gray-700 dark:text-gray-300 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                          {processedResult.formatted_text}
                        </div>
                      )}
                    ...
                    <div className="space-y-2">
                      <div className="flex gap-2">
                        <button 
                          onClick={() => downloadTranscript(false)}
                          className="btn-secondary flex-1"
                          title={`Save transcription (${getModifierKeyLabel()}+S)`}
                        >
                          Download
                          <kbd className="ml-2 px-2 py-0.5 text-xs bg-gray-600/20 rounded">⌘S</kbd>
                        </button>
                        {processedResult.summary && (
                          <button 
                            onClick={() => downloadTranscript(true)}
                            className="btn-secondary px-3"
                            title="Download with summary"
                          >
                            + Summary
                          </button>
                        )}
